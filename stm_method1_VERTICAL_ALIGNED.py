...
class NodeDesignChecker:
    ...
    def __init__(self, fck=27.0, beam_width=500.0):
        self.fck = fck
        self.beam_width = beam_width
        self.classifier = NodeTypeClassifier()
        self.f_cd = 0.85 * fck / 1.5
...
class GradientSTMOptimizer:
    ...
    def __init__(self, user_design: UserSTMDesign, max_change_ratio=0.15):
        ...
        # ═══════════════════════════════════════════════════════════
        self.vertical_pairs = [
            ('B', 'G'),  # B와 G는 항상 같은 x (수직선 유지)
            ('E', 'H')   # E와 H는 항상 같은 x (대칭)
        ]
        ...
        # - C: C의 x 좌표
        # (G는 B와 같은 x, E와 H는 대칭)
        self.left_nodes = ['B', 'C']  # 2개만! (G 제외)
        self.right_nodes = ['E', 'D']
    ...
    def optimize(self, method='L-BFGS-B', verbose=True):
        ...
        if verbose:
            print(f"\n{'='*60}")
            print("Symmetric STM Optimization with Vertical Alignment")
            print(f"{'='*60}")
            print(f"Total nodes: {len(self.node_ids)}")
            print(f"Fixed nodes: {len(self.fixed_nodes)} (A, F - supports)")
            print(f"Optimization variables: {len(self.left_nodes)} X-coordinates (B, C)")
            print(f"Vertical alignment: B-G, E-H (same x)")
    ...
    def _vector_to_nodes(self, x):
        ...
        for i, node_id in enumerate(self.left_nodes):
            x_new = x[i]
            y_fixed = self.initial_nodes[node_id][1]
            nodes[node_id] = (x_new, y_fixed)
        
        # 수직 정렬 제약: G는 B와 같은 x
        x_B = nodes['B'][0]
        y_G = self.initial_nodes['G'][1]
        nodes['G'] = (x_B, y_G)  # B와 같은 x!
        
        # 우측 노드 (대칭)
        for left_id, right_id in self.symmetric_pairs:
            if left_id in nodes:  # 이미 계산된 경우만
    ...
    def _round_to_grid(self, nodes, grid_size=5):
        """노드 위치를 grid_size의 배수로 반올림 (수직 정렬 유지)"""
        rounded_nodes = {}
        ...
        # 수직 정렬: G는 B와 같은 x (반올림된 값)
        x_B = rounded_nodes['B'][0]
        y_G = self.initial_nodes['G'][1]
        rounded_nodes['G'] = (x_B, y_G)
        
        # 우측 노드 대칭 재계산
        for left_id, right_id in self.symmetric_pairs:
            if left_id in rounded_nodes:
    ...
    def _verify_symmetry(self, nodes):
        """대칭 및 수직 정렬 검증 출력"""
        print(f"\n{'='*60}")
        print("Symmetry & Vertical Alignment Verification")
        print(f"{'='*60}")
        ...
        print(f"\n[VERTICAL ALIGNMENT]")
        all_vertical = True
        for node1, node2 in self.vertical_pairs:
            x1, y1 = nodes[node1]
            x2, y2 = nodes[node2]
            x_diff = abs(x1 - x2)
            
            print(f"{node1} - {node2}:")
            print(f"  X-coordinates: {x1:.1f}mm = {x2:.1f}mm")
            
            if x_diff < 1e-6:
                print(f"  ✓ Perfect vertical alignment")
            else:
                print(f"  ⚠️ Alignment error: ΔX={x_diff:.6f}mm")
                all_vertical = False
        ...
        print(f"{'='*60}")
        if all_symmetric and all_vertical:
            print("✓ All constraints satisfied!")
        else:
            if not all_symmetric:
                print("⚠️ Warning: Some symmetry errors detected")
            if not all_vertical:
                print("⚠️ Warning: Some vertical alignment errors detected")
        print(f"{'='*60}\n")
...
def print_summary_statistics(optimizer, result):
    ...
    print(f"  Y-coordinates: FIXED (w_t, w_s determined)")
    print(f"  Optimized variables: {len(optimizer.left_nodes)} X-coordinates (B, C)")
