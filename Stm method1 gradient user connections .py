...
try:
    ...
    USE_EXISTING_CHECKER = True
except ImportError:
    USE_EXISTING_CHECKER = False
    print("⚠️ STM_for_MCP.py not found. Using simplified KDS checker.")
...
class GradientSTMOptimizer:
    ...
    def optimize(self, method='L-BFGS-B', verbose=True):
        ...
        if verbose:
            print(f"\n{'='*60}")
            print("Gradient-based STM Optimization")
            print(f"{'='*60}")
    ...
    def _get_bounds(self):
        ...
        for node_id in self.node_ids:
            ...
            x_max = min(self.user_design.beam_length, x0 + max_dx)
    ...
    def _plot_stm(self, ax, nodes, title):
        ...
        # 부재
        for n1_id, n2_id in self.connections:
            x1, y1 = nodes[n1_id]
            x2, y2 = nodes[n2_id]
            ...
            if dy < 100:
                ...
                color, ls = 'blue', '-'  # Strut (diagonal/vertical)
            
            ax.plot([x1, x2], [y1, y2], color=color, linestyle=ls, linewidth=2.5)
        
        # 노드
        for node_id, (x, y) in nodes.items():
            ax.plot(x, y, 'ko', markersize=8)
            ax.text(x, y+100, node_id, ha='center', fontsize=11, fontweight='bold')
        
        # 지지점
        for node_id, support_type in self.user_design.supports.items():
            x, y = nodes[node_id]
            if support_type == 'pin':
                ax.plot(x, y-80, '^', markersize=12, color='black')
            else:
                ax.plot(x, y-80, 'o', markersize=10, color='black')
        
        # 하중
        for node_id, Fx, Fy in self.user_design.loads:
            x, y = nodes[node_id]
            ax.arrow(x, y+180, 0, -130, head_width=70, head_length=40, fc='darkgreen', ec='darkgreen')
            ax.text(x, y+220, f"{abs(Fy):.0f}kN", ha='center', fontsize=10, color='darkgreen')
...
if __name__ == "__main__":
    ...
    # 시각화
    fig = optimizer.plot_comparison(result, save_path='stm_gradient_comparison.png')
    
    print(f"\n{'='*60}")
    print("✓ Optimization complete!")
    print("✓ Plot saved: stm_gradient_comparison.png")
    print(f"{'='*60}")